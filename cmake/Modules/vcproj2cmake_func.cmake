...
function(_v2c_fs_write_file_unless_existing _file_production_location _content)
  if(EXISTS "${_file_production_location}")
    return()
  endif(EXISTS "${_file_production_location}")
  file(WRITE "${_file_production_location}" "${_content}")
endfunction(_v2c_fs_write_file_unless_existing _file_production_location _content)
  ...
  if(V2C_WANT_SCC_SOURCE_CONTROL_IDE_INTEGRATION)
    ...
    if(bin_tree_somewhere_within_source_)
      if(CMAKE_BINARY_DIR STREQUAL CMAKE_SOURCE_DIR)
        # Example of a candidate which does have problems with such a problematic setup:
...
if(v2c_want_pch_support)
  ...
  function(v2c_target_add_precompiled_header _target _build_platform _build_type _use _header_file _pch_file)
    ...
    # CMAKE_INCLUDE_CURRENT_DIR is similar (but unusable since it activates that
    # for *all* targets).
...
macro(_v2c_target_midl_file_create _file_template _file_production _content)
  ...
  _v2c_create_build_decoupled_adhoc_file("${_file_template}" "${_file_production}" "${_content}")
endmacro(_v2c_target_midl_file_create _file_template _file_production _content)

# Note that these functions make use of some implicitly passed variables.
function(_v2c_target_midl_header_file_create_dummy _target _header_file_location)
  ...
  # This include guard name does not match the one
...
\#endif /* ${midl_header_include_guard_} */")
  _v2c_target_midl_file_create("${header_template_}" "${_header_file_location}" "${header_content_}")
endfunction(_v2c_target_midl_header_file_create_dummy _target _header_file_location)
function(_v2c_target_midl_iid_file_create_dummy _target _iid_file)
...
"
  ...
  _v2c_target_midl_file_create("${iidfile_template_}" "${_iid_file}" "${iidfile_content_}")
endfunction(_v2c_target_midl_iid_file_create_dummy _target _iid_file)

function(_v2c_target_midl_proxy_file_create_dummy _target _proxy_file)
  set(proxy_template_ "${CMAKE_CURRENT_BINARY_DIR}/midl_proxy_${_target}_p.c.in")
  set(proxy_content_ "")
  _v2c_target_midl_file_create("${proxy_template_}" "${_proxy_file}" "${proxy_content_}")
endfunction(_v2c_target_midl_proxy_file_create_dummy _target _proxy_file)

function(_v2c_target_midl_dlldata_file_create_dummy _target _dlldata_file)
  ...
  # HOWEVER, that check might fail since it's configure-time rather than
  # build-time, thus build-time parts such as CMAKE_CFG_INTDIR will break it.
  # IOW, it's high time that we move all MIDL processing to build time.
  if(EXISTS "${_dlldata_file}")
    return()
  endif(EXISTS "${_dlldata_file}")
  set(dlldata_template_ "${CMAKE_CURRENT_BINARY_DIR}/midl_dlldata_${_target}_p.c.in")
  set(dlldata_content_ "")
  _v2c_target_midl_file_create("${dlldata_template_}" "${_dlldata_file}" "${dlldata_content_}")
endfunction(_v2c_target_midl_dlldata_file_create_dummy _target _dlldata_file)

### MIDL PER-MODE SWITCHEROO ###

macro(_v2c_target_midl_do_compile_windows_dummy)
...
endmacro(_v2c_target_midl_do_compile_windows_dummy)

macro(_v2c_target_midl_do_compile_wine_widl)
...
endmacro(_v2c_target_midl_do_compile_wine_widl)

macro(_v2c_target_midl_do_compile_emulated_stubs)
...
")
  if(header_file_location_)
    _v2c_target_midl_header_file_create_dummy(${_target} "${header_file_location_}")
  endif(header_file_location_)
  if(v2c_target_midl_compile_INTERFACE_IDENTIFIER_FILE_NAME)
    _v2c_target_midl_iid_file_create_dummy(${_target} "${v2c_target_midl_compile_INTERFACE_IDENTIFIER_FILE_NAME}")
  endif(v2c_target_midl_compile_INTERFACE_IDENTIFIER_FILE_NAME)
  if(v2c_target_midl_compile_PROXY_FILE_NAME)
    _v2c_target_midl_proxy_file_create_dummy(${_target} "${v2c_target_midl_compile_PROXY_FILE_NAME}")
  endif(v2c_target_midl_compile_PROXY_FILE_NAME)
  # It is said that unspecified DLL_DATA_FILE_NAME parm
  # will cause it to use a dlldata.c default.
  set(midl_dlldata_file_name_ "dlldata.c")
  if(v2c_target_midl_compile_DLL_DATA_FILE_NAME)
    set(midl_dlldata_file_name_ "${v2c_target_midl_compile_DLL_DATA_FILE_NAME}")
  endif(v2c_target_midl_compile_DLL_DATA_FILE_NAME)
  _v2c_target_midl_dlldata_file_create_dummy(${_target} "${midl_dlldata_file_name_}")
endmacro(_v2c_target_midl_do_compile_emulated_stubs)

macro(_v2c_target_midl_do_compile_user_callback)
...
endmacro(_v2c_target_midl_do_compile_user_callback)
...
function(_v2c_midl_compiler_mode_choose _mode)
  ...
  elseif(${_mode} STREQUAL ${v2c_midl_handling_mode_user_callback_})
    macro(_v2c_target_midl_do_compile)
      _v2c_target_midl_do_compile_user_callback()
    endmacro(_v2c_target_midl_do_compile)
  else()
...
function(v2c_target_pdb_configure _target _build_platform _build_type)
  ...
  if(is_active_)
    ...
    if(v2c_target_pdb_configure_PDB_NAME)
      # TODO: should also support the currently Ninja-specific CMAKE_DEBUG_SYMBOL_SUFFIX
      # setting if possible.
    ...
    if(v2c_target_pdb_configure_PDB_OUTPUT_DIRECTORY)
      ...
      # (nope, it *is* doing that, via split values: "PDB" + "_OUTPUT_DIRECTORY")
...
function(v2c_target_post_setup _target _project_label _vs_keyword)
  if(TARGET ${_target})
    ...
    # both project() _and_ PROJECT_LABEL may obviously
    # reference the same project name sometimes.
