...
# Helper to escape a backslash for certain uses,
# to avoid annoying CMake parser errors such as
# "syntax error, unexpected cal_SYMBOL, expecting $end (513)"
macro(_v2c_char_escaped_backslash_get _out_char)
  # I really don't know why one would need that many, but...
  set(${_out_char} "\\\\\\\\")
endmacro(_v2c_char_escaped_backslash_get _out_char)
...
macro(
  _v2c_create_build_decoupled_adhoc_file
  _file_template
  _file_production
  _content
)
...
# Some mechanisms (e.g. MIDL) are supposed to write to a file
# only in case a (custom-made!) version does *not* exist.
# Note that such a file existence check
# prior to carrying on
# is problematic, though:
# in case of file existence
# it will prevent file writing
# not only in case of a *custom*-made file,
# but also in case of a file
# which has been written
# by a *prior* version of the converter implementation -
# IOW the new implementation
# will *not* be able
# to get its implementation improvements
# applied to the file.
# Thus, such a file existence mechanism
# best is to be avoided
# wherever things can be implemented in a superiour way.
function(
  _v2c_fs_write_file_unless_existing
  _file_production_location
  _content
)
  if(EXISTS
    "${_file_production_location}"
...
function(
  _v2c_autogenerated_text_get
  _out_comment
)
  set(${_out_comment}
    "Autogenerated by vcproj2cmake - Do not edit."
    PARENT_SCOPE
  )
endfunction(
  _v2c_autogenerated_text_get
...
function(
  _v2c_autogenerated_text_c_comment_get
  _out_c_comment
)
  _v2c_autogenerated_text_comment_get(
    v2c_autogenerated_text_comment_
  )
  set(autogenerated_stars_
    "**"
  )
  set(v2c_autogenerated_text_c_comment_
    "/*${autogenerated_stars_} ${v2c_autogenerated_text_comment_} ${autogenerated_stars_}*/"
  )
  set(${_out_c_comment}
    "${v2c_autogenerated_text_c_comment_}"
    PARENT_SCOPE
  )
endfunction(
  _v2c_autogenerated_text_c_comment_get
...
function(_v2c_scc_do_setup)
  ...
  if(V2C_WANT_SCC_SOURCE_CONTROL_IDE_INTEGRATION)
    set(prefix_of_source_dir_ "${CMAKE_SOURCE_DIR}/")
    string(LENGTH "${prefix_of_source_dir_}" src_len_)
    string(SUBSTRING "${CMAKE_BINARY_DIR}/" 0 ${src_len_} bin_test_)
    set(bin_tree_somewhere_within_source_ OFF)
    if("${bin_test_}" STREQUAL "${prefix_of_source_dir_}")
      set(bin_tree_somewhere_within_source_ ON)
    endif("${bin_test_}" STREQUAL "${prefix_of_source_dir_}")
    if(bin_tree_somewhere_within_source_)
      if(CMAKE_BINARY_DIR STREQUAL CMAKE_SOURCE_DIR)
        # Example of a candidate
        # which does have problems with such a problematic setup:
        # Eclipse (according to a check in CMake's generator source).
        _v2c_msg_warning("CMAKE_BINARY_DIR *equal to* CMAKE_SOURCE_DIR! This is definitely *not* recommended!! (the build tree should always be in a separate directory, ideally out-of-tree, best as a *sibling* to the source root)")
...
# Note that rather than post-ensuring proper correctness
# via checks with this helper,
# one usually should prefer simply fetching values
# via a maximally direct local-variable query
# from a properly and reliably function-scoped helper.
...
option(V2C_WANT_FILELIST_TYPE_DEVIATION_SUPPORT
  "Enable support of project filelist compiler language type deviation"
  ON
)
option(V2C_WANT_PDB_SUPPORT
  "Enable support of PDB (Program DataBase) debug information files"
  ON
)
option(V2C_WANT_INSTALL_SUPPORT
  "Activate infrastructure for configurable installation of V2C targets"
  ON
)
...
function(
  v2c_project_target_current_get
  _out_target
)
  get_property(
    project_target_current_
    DIRECTORY
    PROPERTY
    V2C_PROJECT_TARGET_CURRENT
  )
...
function(_v2c_temp_store_dir_relpath_get _out_temp_store_dir_relpath)
  ...
  set(temp_dir_ "${cfg_dir_}/temporary_scm_ignored_content")
...
# Returns the directory which is used to store items generated by the converter,
# in *source* tree. While *binary* tree would be preferable since it's
# not a good idea to fumble a static source tree, the conversion run does happen
# before any build tree is in place (and does create something semi-static),
# thus the resulting files (and only those!)
# do need to be placed in source tree after all.
function(_v2c_fs_generated_items_dir_relpath_get _out_generated_items_dir_relpath)
...
_v2c_var_my_get(WANT_FILELIST_TYPE_DEVIATION_SUPPORT
  v2c_want_filelist_type_deviation_support
)
...
if(v2c_want_pch_support)
  ...
  function(v2c_target_add_precompiled_header _target _build_platform _build_type _use _header_file _pch_file)
    ...
    # CMAKE_INCLUDE_CURRENT_DIR is similar
    # (but unusable
    # since it activates that for *all* targets).
...
macro(_v2c_target_linker_def_file
  _target
  _build_platform
  _build_type
  _def_file
  ...
  if(COMMAND v2c_user_callback_target_linker_def_file_v1)
    ...
    v2c_user_callback_target_linker_def_file_v1(
      ${_target}
      "${_build_platform}"
      "${_build_type}"
      "${_def_file}"
...
endmacro(_v2c_target_linker_def_file
  _target
  _build_platform
  _build_type
  _def_file
...
function(_v2c_midl_compiler_mode_get _out_mode)
  ...
  set(v2c_midl_doc_string_ "The mode to use for handling of IDL files [this string should be one of: ${v2c_midl_handling_mode_windows_dummy_} - uses builtin Windows MIDL handling / ${v2c_midl_handling_mode_wine_widl_} - uses Wine's widl IDL compiler / ${v2c_midl_handling_mode_emulated_stubs_} - tries to come up with a sufficiently complete emulation stub to at least allow a successful project build [Code Coverage!] / ${v2c_midl_handling_mode_user_callback_} - invokes a user callback if existing, to allow for user-side custom implementations of MIDL compiler]")
...
macro(
  _v2c_target_tool_midl_file_create
  _file_template
  _file_production
  _content
)
  # TODO: some of that MIDL processing should probably be moved build-time,
...
function(_v2c_target_tool_midl_header_file_create_dummy _target _header_file_location)
  ...
  # This include guard name
  # does not match the one
  # usually used by MIDL compilers (seems to be __ProjectName_h__),
...
function(_v2c_target_tool_midl_iid_file_create_dummy _target _iid_file)
  ...
  _v2c_char_escaped_backslash_get(escaped_bs_) # Oh well...
  _v2c_autogenerated_text_c_comment_get(
    midl_autogenerated_text_header_
  )
...
\#define MIDL_DEFINE_GUID(type,name,l,w1,w2,b1,b2,b3,b4,b5,b6,b7,b8) ${escaped_bs_}
...
function(
  _v2c_target_tool_midl_proxy_file_create_dummy
  _target
  _proxy_file
)
  set(proxy_template_
    "${CMAKE_CURRENT_BINARY_DIR}/midl_proxy_${_target}_p.c.in")
  set(proxy_content_ "")
  _v2c_target_tool_midl_file_create(
    "${proxy_template_}"
    "${_proxy_file}"
...
function(
  _v2c_target_tool_midl_dlldata_file_create_dummy
  _target
  _dlldata_file
)
  # "Each MIDL compiler run for COM interfaces
  # looks for an existing dlldata.c file first,
  # and if the file is not found, a new dlldata.c file is created"
  # So, this means we need a file existence check here.
  # HOWEVER, that check might fail
  # since it's configure-time rather than build-time,
  # thus complex conditional build-time parts
  # (example: CMAKE_CFG_INTDIR)
  # will break it.
  # IOW, it's high time that we move all MIDL processing to build time.
  if(EXISTS "${_dlldata_file}")
    return()
  endif(EXISTS "${_dlldata_file}")
  set(dlldata_template_
    "${CMAKE_CURRENT_BINARY_DIR}/midl_dlldata_${_target}_p.c.in")
  set(dlldata_content_
    "")
  _v2c_target_tool_midl_file_create(
    "${dlldata_template_}"
    "${_dlldata_file}"
...
macro(_v2c_target_tool_midl_do_compile_windows_dummy)
  ...
  _v2c_target_tool_midl_include_output_directory(
    ${_target}
  )
endmacro(_v2c_target_tool_midl_do_compile_windows_dummy)

macro(_v2c_target_tool_midl_do_compile_wine_widl)
  _v2c_var_my_get(WINE_WIDL_BIN wine_widl_bin_)
  set(cmd_list_ "${wine_widl_bin_}")
  set(v2c_widl_depends_ "${wine_widl_bin_}")
  ...
  _v2c_var_my_get(WINE_WINDOWS_INCLUDE_DIR wine_windows_include_dir_)
  ...
  if(v2c_target_tool_midl_compile_TYPE_LIBRARY_NAME)
    list(APPEND cmd_list_ "-t" "-T${v2c_target_tool_midl_compile_TYPE_LIBRARY_NAME}")
    # Hmm, it appears that widl (at least certain older versions of it)
    ...
    #list(APPEND v2c_widl_outputs_ "${v2c_target_tool_midl_compile_TYPE_LIBRARY_NAME}")
  endif(v2c_target_tool_midl_compile_TYPE_LIBRARY_NAME)
  if(v2c_target_tool_midl_compile_PROXY_FILE_NAME)
    list(APPEND cmd_list_ "-p" "-P${v2c_target_tool_midl_compile_PROXY_FILE_NAME}")
    list(APPEND v2c_widl_outputs_ "${v2c_target_tool_midl_compile_PROXY_FILE_NAME}")
  endif(v2c_target_tool_midl_compile_PROXY_FILE_NAME)
  if(v2c_target_tool_midl_compile_DLL_DATA_FILE_NAME)
    list(APPEND cmd_list_ "--dlldata=${v2c_target_tool_midl_compile_DLL_DATA_FILE_NAME}")
  endif(v2c_target_tool_midl_compile_DLL_DATA_FILE_NAME)
  if(v2c_target_tool_midl_compile_VALIDATE_ALL_PARAMETERS)
    # Not sure at all whether v2c_target_tool_midl_compile_VALIDATE_ALL_PARAMETERS is even
    # related to "enable pedantic warnings"...
    list(APPEND cmd_list_ "-W")
  endif(v2c_target_tool_midl_compile_VALIDATE_ALL_PARAMETERS)
  if(v2c_target_tool_midl_compile_TARGET_ENVIRONMENT)
    if(v2c_target_tool_midl_compile_TARGET_ENVIRONMENT STREQUAL "Win32")
      list(APPEND cmd_list_ "--win32")
    endif(v2c_target_tool_midl_compile_TARGET_ENVIRONMENT STREQUAL "Win32")
    # Note that we don't support "Itanium" here yet.
    if(v2c_target_tool_midl_compile_TARGET_ENVIRONMENT STREQUAL "X64")
      list(APPEND cmd_list_ "--win64")
    endif(v2c_target_tool_midl_compile_TARGET_ENVIRONMENT STREQUAL "X64")
  ...
  if(v2c_widl_outputs_)
    ...
    add_custom_target(${_target}_tool_midl_compile DEPENDS ${v2c_widl_outputs_})
    add_dependencies(${_target} ${_target}_tool_midl_compile)
  else(v2c_widl_outputs_)
    _v2c_msg_warning("${_target}: MIDL implementation problem - no outputs!? Bailing out...")
  endif(v2c_widl_outputs_)
endmacro(_v2c_target_tool_midl_do_compile_wine_widl)

macro(_v2c_target_tool_midl_do_compile_emulated_stubs)
...
")
  ...
  if(v2c_target_tool_midl_compile_PROXY_FILE_NAME)
    _v2c_target_tool_midl_proxy_file_create_dummy(
      ${_target}
      "${v2c_target_tool_midl_compile_PROXY_FILE_NAME}"
    )
  endif(v2c_target_tool_midl_compile_PROXY_FILE_NAME)
  # It is said that unspecified DLL_DATA_FILE_NAME parm
  # will cause it to use a dlldata.c default.
  set(midl_dlldata_file_name_
    "dlldata.c")
  if(v2c_target_tool_midl_compile_DLL_DATA_FILE_NAME)
    set(midl_dlldata_file_name_
      "${v2c_target_tool_midl_compile_DLL_DATA_FILE_NAME}")
  endif(v2c_target_tool_midl_compile_DLL_DATA_FILE_NAME)
  _v2c_target_tool_midl_dlldata_file_create_dummy(
    ${_target}
    "${midl_dlldata_file_name_}"
  )
endmacro(_v2c_target_tool_midl_do_compile_emulated_stubs)

macro(_v2c_target_tool_midl_do_compile_user_callback)
  if(COMMAND v2c_user_callback_midl_compile_v1)
    # Note that the user is expected to make use
    # of the argument variables created by the outer parse_arguments().
    v2c_user_callback_midl_compile_v1()
  else(COMMAND v2c_user_callback_midl_compile_v1)
    _v2c_msg_warning("MIDL mode chosen to be user callback, yet no callback function provided!?")
  endif(COMMAND v2c_user_callback_midl_compile_v1)
endmacro(_v2c_target_tool_midl_do_compile_user_callback)

# Chooses the macro to be implemented
# according to the MIDL compiler mode passed in.
function(_v2c_midl_compiler_mode_choose _mode)
  _v2c_var_ensure_defined(v2c_midl_handling_mode_windows_dummy_ v2c_midl_handling_mode_wine_widl_ v2c_midl_handling_mode_emulated_stubs_)
  if(${_mode} STREQUAL ${v2c_midl_handling_mode_windows_dummy_})
    macro(_v2c_target_tool_midl_do_compile)
      _v2c_target_tool_midl_do_compile_windows_dummy()
    endmacro(_v2c_target_tool_midl_do_compile)
  elseif(${_mode} STREQUAL ${v2c_midl_handling_mode_wine_widl_})
    macro(_v2c_target_tool_midl_do_compile)
      _v2c_target_tool_midl_do_compile_wine_widl()
    endmacro(_v2c_target_tool_midl_do_compile)
  elseif(${_mode} STREQUAL ${v2c_midl_handling_mode_emulated_stubs_})
    macro(_v2c_target_tool_midl_do_compile)
      _v2c_target_tool_midl_do_compile_emulated_stubs()
    endmacro(_v2c_target_tool_midl_do_compile)
  elseif(${_mode} STREQUAL ${v2c_midl_handling_mode_user_callback_})
    macro(_v2c_target_tool_midl_do_compile)
      _v2c_target_tool_midl_do_compile_user_callback()
    endmacro(_v2c_target_tool_midl_do_compile)
  else()
    _v2c_msg_fatal_error("Unknown MIDL handling mode ${_mode}")
  endif(${_mode} STREQUAL ${v2c_midl_handling_mode_windows_dummy_})
...
function(_v2c_midl_compiler_mode_do_setup)
  ...
  set(v2c_midl_handling_mode_windows_dummy_ "Windows")
  set(v2c_midl_handling_mode_wine_widl_ "Wine")
  set(v2c_midl_handling_mode_emulated_stubs_ "EmulatedStubs")
  set(v2c_midl_handling_mode_user_callback_ "UserCallback")
...
function(v2c_target_tool_midl_compile _target _build_platform _build_type)
  ...
  v2c_parse_arguments(v2c_target_tool_midl_compile "${options}" "${oneValueArgs}" "${multiValueArgs}" ${ARGN})

  ## COMMON HANDLING ##
  _v2c_fs_item_make_relative_to_path("${v2c_target_tool_midl_compile_IDL_FILE_NAME}" "${PROJECT_SOURCE_DIR}" idl_file_location_)
...
endfunction(v2c_target_tool_midl_compile _target _build_platform _build_type)

_v2c_var_my_get(WANT_PDB_SUPPORT
  _v2c_want_pdb_support
)
...
function(v2c_target_pdb_configure _target _build_platform _build_type)
  ...
  if(is_active_)
    ...
    if(v2c_target_pdb_configure_PDB_OUTPUT_DIRECTORY)
      ...
      # (nope, it *is* doing that, via split values:
      # "PDB" + "_OUTPUT_DIRECTORY")
...
function(v2c_target_install _target)
  ...
  foreach(install_param_ ${install_param_list_})
    ...
    if(install_param_value_)
      ...
      if(install_param_ STREQUAL DESTINATION)
        _v2c_msg_fatal_error("Installation mode activated, yet for target ${_target} (type ${target_type_}) mandatory install param ${install_param_} could not be figured out (some possible reasons: variable V2C_INSTALL_${install_param_}_${_target} or V2C_INSTALL_${install_param_} not specified)")
      endif(install_param_ STREQUAL DESTINATION)
...
function(v2c_target_post_setup _target _project_label _vs_keyword)
  if(TARGET ${_target})
    ...
    # Since PROJECT_LABEL is used for IDE-side project name customization,
    # both project() _and_ PROJECT_LABEL
    # may obviously reference the same project name sometimes.
