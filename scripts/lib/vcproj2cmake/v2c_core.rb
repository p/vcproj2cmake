...
def check_cmakelists_txt_type(str_cmakelists_file_fqpn)
  ...
  begin
    f_cmakelists = File.new(str_cmakelists_file_fqpn, 'r')
...
def v2c_source_root_write_projects_list_file(output_file_fqpn, create_permissions, arr_project_subdirs)
  ...
  begin
    ...
    textOut = V2C_TextStreamSyntaxGeneratorBase.new(projlistfile, $v2c_generator_indent_initial_num_spaces, $v2c_generator_indent_step, $v2c_generator_comments_level)
...
class V2C_CMakeSkeletonFileGenerator < V2C_CMakeV2CSyntaxGenerator
  ...
  def generate
    @textOut.put_file_header_temporary_marker
    write_comment_at_level(COMMENT_LEVEL_MINIMUM, 
      "vcproj2cmake.sf.net source root skeleton file.\n" \
...
def v2c_source_root_write_cmakelists_skeleton_file(path_cmakelists_txt, projects_list_file)
  File.open(path_cmakelists_txt, 'w') { |f_cl|
    textOut = V2C_TextStreamSyntaxGeneratorBase.new(f_cl, $v2c_generator_indent_initial_num_spaces, $v2c_generator_indent_step, $v2c_generator_comments_level)
    skeleton_generator = V2C_CMakeSkeletonFileGenerator.new(textOut, projects_list_file)
...
def v2c_source_root_ensure_usable_cmakelists_skeleton_file(source_root, projects_list_file)
  ...
  create_skeleton_root_file = false
  case root_cmakelists_txt_type
  when CMAKELISTS_FILE_TYPE_NONE, CMAKELISTS_FILE_TYPE_ZERO_SIZE, CMAKELISTS_FILE_TYPE_V2C_ROOT
    create_skeleton_root_file = true
  when CMAKELISTS_FILE_TYPE_CUSTOM, CMAKELISTS_FILE_TYPE_V2C_LOCAL
    create_skeleton_root_file = false
  else
    raise V2C_GeneratorError, 'unknown/unsupported/corrupt CMakeLists.txt type value!'
  end
  
  if true == create_skeleton_root_file
    begin
      log_info "creating/updating an initially usable skeleton file at #{root_cmakelists_txt_file}"
