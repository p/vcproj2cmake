...
# What we're interested in probably is number of cores, even in an SMT (HT)
# case.
...
class V2C_Item_List_Info
  ...
  def initialize(name, type = TYPE_NONE)
    ...
    @name = name
    @type = type
    @arr_items = Array.new # V2C_Info_File elements
    @hash_items = Hash.new # V2C_Info_File elements
  end
  attr_accessor :name
  attr_accessor :type
  ...
  def append_item(file_info, flags)
    ...
    existing_item = @hash_items[path_new]
    ...
    if existing_item.nil?
      @arr_items.push(file_info)
      @hash_items[path_new] = file_info
    else
      existing_item.extend(file_info)
    end
  end
  def get_item(item_name)
    @arr_items.each { |info_item|
      #puts "#{item_name} vs. #{info_item.path_relative}"
      if item_name.eql?(info_item.path_relative)
        return info_item
      end
    }
    return nil
  ...
  def get_generated_items
    array_collect_compact(@arr_items) do |item_info|
      #puts "#{item_info.path_relative} #{item_info.is_generated}"
      next if false == item_info.is_generated
      item_info.path_relative
    end
  end
  private
  def get_type(type); @type <= TYPE_NONE ? @type : TYPE_NONE end
  def get_list_type_descr()
    ITEM_LIST_DESCRIPTIONS[get_type(type)]
  end
end
...
class V2C_Item_Lists_Container
  ...
  def lookup_from_list_name(item_list_name)
    return @hash_item_lists[item_list_name]
  end
  def lookup_from_list_type(item_list_type)
    @arr_item_lists.each { |list|
      return list if item_list_type == list.type
    }
    return nil
  end
  def lookup_from_item_name(item_name)
    info_item = nil
  ...
  def append(item_list)
    name = item_list.name
    item_list_existing = lookup_from_list_name(name)
  ...
  def register(item_list)
    @arr_item_lists.push(item_list)
    @hash_item_lists[item_list.name] = item_list
  end
end
...
def cmake_path_combine(arr_prefix, arr_suffix)
  arr_paths = Array.new
...
class V2C_XmlParserBase < V2C_ParserBase
  ...
  def string_unescape_from_xml__extended(raw)
    ...
    cooked = string_xml_unescape_predefined_entities(raw)
    ...
    #   % escapes
    # - this might be a transcoding transition layer that's different from the
    #   layer of the usual XML entities escaping transition (especially
    #   since docs say that it's MSBuild where the '%' sign is "reserved")
    ...
    # (i.e., that would translate commands / parameters / environment vars)
    cooked.gsub!('%25', "%")
    cooked
  end
  # Translates the 5 well-known predefined XML entities
  def string_xml_unescape_predefined_entities(raw)
...
class V2C_VSToolPrePostBuildLinkEventParser < V2C_VSToolParserBase
  ...
  def parse_commandline(str_in)
    ...
    array_collect_compact(arr_commandline_raw) do |commandline_raw|
      str_unescaped = string_unescape_from_xml__extended(string_value_preprocess(commandline_raw))
  ...
  def parse
    ...
    if not elem_first.nil?
      ...
      when 'Filter'
        ...
        elem_parser = V2C_VS10ItemGroupFilesParser.new(@elem_xml, elem_name, get_project().item_lists)
      end
      if not elem_parser.nil?
        found = elem_parser.parse
      else
...
class V2C_CMakeSyntaxGenerator < V2C_SyntaxGeneratorBase
  ...
  def add_subdirectory(str_subdir_source, str_subdir_binary = nil, exclude_from_all = false)
    # quote strings containing spaces!!
    str_subdir_source_quoted = element_handle_quoting(str_subdir_source)
    arr_args = [ str_subdir_source_quoted ]
    if not str_subdir_binary.nil?
      str_subdir_binary_quoted = element_handle_quoting(str_subdir_binary)
      arr_args.push(str_subdir_binary_quoted)
    end
    if false != exclude_from_all
      arr_args.push('EXCLUDE_FROM_ALL')
    end
    write_command_list_single_line('add_subdirectory', arr_args)
  end
  ...
  def put_property_source(arr_file_elems, prop_key, arr_prop_vals)
    arr_arg_sources = [ 'SOURCE' ]
    arr_arg_sources.concat(arr_file_elems)
    put_property(arr_arg_sources, PROP_SET, prop_key, arr_prop_vals)
  end
  ...
  def put_file_list_marked_as_generated(file_list_description, arr_generated_files, is_generated)
    file_list_var = "SOURCES_GENERATED_#{file_list_description}"
    write_list_quoted(file_list_var, arr_generated_files)
    arr_file_elems = [ get_dereferenced_variable_name(file_list_var) ]
    file_list_mark_as_generated(arr_file_elems, is_generated)
  end
...
class V2C_CMakeFileListGenerator_VS10 < V2C_CMakeFileListGeneratorBase
  ...
  def generate_file_list(file_list)
    arr_local_sources = filter_files(file_list.arr_items)
...
class V2C_CMakeLinkerInfoGenerator < V2C_CMakeTargetGenerator
  ...
  def generate(condition, linker_info_curr, target_config_info_curr)
    generate_linker_info(condition, @target_name, linker_info_curr, target_config_info_curr)
  end

  private
  def generate_linker_info(condition, target_name, linker_info_curr, target_config_info_curr)
...
class V2C_CMakeCompilerInfoGenerator < V2C_CMakeTargetGenerator
  ...
  def put_property_compile_definitions(condition, arr_compile_defn)
    ...
    set_property_per_config(condition, get_target_name(), PROP_APPEND, 'COMPILE_DEFINITIONS', arr_compile_defn_cooked)
  end
  ...
  def write_property_compile_flags(condition, arr_flags, arr_conditional)
    ...
    gen_condition.generate(condition) do
      write_conditional_block(arr_conditional) do
        ...
        set_property_per_config(condition, get_target_name(), PROP_APPEND, 'COMPILE_FLAGS', arr_flags)
      end
    end
  end
  ...
  def configure_pdb(condition, pdb_info)
    put_v2c_target_pdb_configure(get_target_name(), condition, pdb_info)
  end
end
...
class V2C_CMakeProjectTargetGenerator < V2C_CMakeTargetGenerator
  ...
  def put_file_list(project_info, arr_sub_source_list_var_names)
    ...
    put_file_list_vs10(project_info.name, project_info.item_lists, arr_sub_source_list_var_names)
  ...
  def put_file_list_vs10(project_name, file_lists, arr_sub_sources_for_parent)
    ...
    file_lists.arr_item_lists.each { |file_list|
      ...
      arr_generated = file_list.get_generated_items
  ...
  def file_list_mark_as_generated(arr_file_elems, is_generated)
    put_property_source_bool(arr_file_elems, 'GENERATED', is_generated)
  end
  ...
  def hook_up_tools(item_lists, config_info)
    arr_tool_context = Array.new
    item_list_midl = item_lists.lookup_from_list_type(V2C_Item_List_Info::TYPE_MIDL)
    ...
    config_info.tools.arr_midl_info.each do |midl_info|
      ...
      arr_tool_context.push(TOOL_CONTEXT_STR.new(V2C_ToolProcessorMIDL.new(midl_info), item_list_midl, 0)) # FIXME add tool types
    end
    ...
    arr_tool_context.each do |tool_context|
      ...
      do_hookup_tools(get_target_name(), config_info.condition, tool_context)
    end
  end
  def do_hookup_tools(target_name, condition, tool_context)
    ...
    arr_input_items = file_list_input.arr_items
    arr_input_items.each do |input_file|
  ...
  def put_atl_mfc_config(target_config_info)
    ...
    do_configure_atl_mfc_flag(get_target_name(), target_config_info.condition, target_config_info.use_of_atl, target_config_info.use_of_mfc)
  end
  ...
  def put_target_and_stuff(target, arr_sub_source_list_var_names, map_lib_dirs, map_lib_dirs_dep, map_dependencies, config_info_curr, target_config_info_curr)
    ...
    config_info_curr.tools.arr_linker_info.each { |linker_info_curr|
      ...
      generate_module_definition_file_call(project_target_name, config_info_curr.condition, linker_info_curr.module_definition_file)
    }
    ...
    config_info_curr.tools.arr_linker_info.each { |linker_info_curr|
      @localGenerator.write_build_attributes('target_link_libraries', linker_info_curr.arr_lib_dirs, map_lib_dirs_dep, project_target_name, true)
    }

    config_info_curr.tools.arr_prepostbuildlink_info.each { |prepostbuild_info_curr|
      put_prepostbuildlink_event(project_target_name, prepostbuild_info_curr)
    }
  ...
  def write_target_type(cfg_type)
    ...
    if target_is_valid
      write_command_single_line('_v2c_project_target_current_update', target_name)
    end
    return target_is_valid
  end
  def write_WinMain()
    arr_target_prop = get_target_syntax_expression(get_target_name())
  ...
  def write_property_output_name(condition, output_name)
    gen_condition = V2C_CMakeV2CConditionGenerator.new(@textOut, false)
    gen_condition.generate(condition) do
      set_property_per_config(condition, get_target_name(), PROP_SET, 'OUTPUT_NAME', [ output_name ])
      next_paragraph()
    end
  end
  def write_link_libraries(arr_dependencies, map_dependencies)
    ...
    @localGenerator.write_build_attributes('target_link_libraries', arr_dependencies_augmented, map_dependencies, get_target_name())
  end
  def write_func_v2c_target_post_setup(project_name, project_keyword)
    ...
    write_invoke_config_object_v2c_function_quoted('v2c_target_post_setup', get_target_name(), arr_args_func)
  end
  ...
  def generate_it(generator_base, map_lib_dirs, map_lib_dirs_dep, map_dependencies, map_defines)
    ...
    # environment).
    if target_is_valid
      when_target_valid_scriptlet_block(get_target_name()) {
        ...
        arr_config_info.each { |config_info_curr|
          ...
          project_info.arr_target_config_info.each { |target_config_info_curr|
            ...
            compiler_info_generator = V2C_CMakeCompilerInfoGenerator.new(@textOut, get_target_name())
            ...
            tools.arr_linker_info.each { |linker_info_curr|
              ...
              linker_info_generator.generate(condition, linker_info_curr, target_config_info_curr)
    ...
    if target_is_valid
      ...
      write_func_v2c_target_post_setup(target_name, project_info.vs_keyword)

      if project_info.project_types != nil
        set_property_project_types(target_name, project_info.project_types)
      end
      if project_info.user_properties.length > 0
        set_properties_user_properties(target_name, project_info.user_properties)
      end

      set_properties_vs_scc(target_name, project_info.scc_info)
    ...
    write_func_v2c_project_post_setup(get_project_name(), arr_orig_proj_files)
  end
  ...
  def put_property_dotnet_references(target_name, arr_references)
    set_property(target_name, PROP_SET, 'VS_DOTNET_REFERENCES', arr_references)
  end
  ...
  def write_project(project_info)
    ...
    put_project(get_project_name(), arr_languages)
  end
...
# since e.g. compiler info is dependent on a valid project.
  ...
  def generate_project_leadin(project_info)
    ...
    put_conversion_info(project_name, project_info.orig_environment_shortname)
    put_guid(project_name, project_info.guid)
...
class V2C_CMakeGlobalBootstrapCodeGenerator < V2C_CMakeV2CSyntaxGenerator
  ...
  def put_cmake_module_path_bootstrap(arr_module_paths_relative)
    write_comment_at_level(COMMENT_LEVEL_STANDARD,
    ...
    arr_paths = cmake_path_combine(arr_cmake_refs, arr_module_paths_relative)
  ...
  def put_cmake_module_path(str_conversion_root_rel)
    ...
    put_cmake_module_path_bootstrap(arr_module_paths_relative)
  end
...
V2C_Subdir_Info = Struct.new(:source_dir, :binary_dir)

def v2c_source_root_write_projects_list_file(output_file_fqpn, output_file_permissions, arr_project_subdir_infos)
  ...
  generate_projects_list.generate { |textOut|
    ...
    arr_project_subdir_infos.each { |subdir_info|
      projects_list_generator.add_subdirectory(subdir_info.source_dir, subdir_info.binary_dir)
    }
  }
rescue Exception
...
def v2c_projects_list_handle_sub_dirs(p_source_root, arr_project_subdir_infos)
  ...
  v2c_source_root_write_projects_list_file(projects_list_file, $v2c_generator_file_create_permissions, arr_project_subdir_infos)
end
...
class V2C_ProjectPostProcess < V2C_LoggerBase
  ...
  def process
    mark_items_as_generated(@project_info.item_lists, @project_info.arr_config_info)
  ...
  def mark_items_as_generated(item_lists, arr_config_info)
    ...
    arr_list_types = [ V2C_Item_List_Info::TYPE_CL_COMPILES, V2C_Item_List_Info::TYPE_CL_INCLUDES ]
    ...
    arr_output_items.each do |candidate|
      arr_lists_build.each do |item_list_build|
        ...
        if info_item.nil?
          item_list_build.arr_items.each do |item|
            logger.debug "item is #{item.path_relative} candidate #{candidate}"
            if item.path_relative == candidate
              info_item = item
              break
            end
          end
        ...
        if not info_item.nil?
          logger.info "Marking item #{info_item.path_relative} as generated (reason: output item of build tool #{build_tool_name})"
          info_item.enable_attribute(V2C_Info_File::ATTR_GENERATED)
        end
  ...
  def detect_build_units(project_info)
    if not true == project_info.have_build_units
      project_info.item_lists.arr_item_lists.each { |file_list|
        arr_file_infos = file_list.arr_items
