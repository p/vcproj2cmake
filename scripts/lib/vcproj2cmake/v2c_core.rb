...
module V2C_File_Stuff
  ...
  def fs_location_get_filename_part_wo_ext(
    location)
    ext = File.extname(
      location)
    File.basename(
...
# that can be specified per-configuration
# (target type [lib/exe], include directories)
# but where CMake unfortunately
# does _NOT_ offer a configuration-specific equivalent,
# we need to fall back to using the globally-scoped CMake commands
# (include_directories() etc.).
# But at least let's optionally allow the user
# to precisely specify which configuration
...
# to have log channel provided
# as a class member or even a global variable.
# Should thus rework things to have a class member each
...
if (RUBY_VERSION < '1.9') # FIXME exact version where it got introduced?
  ...
  def hash_ensure_sorted(hash_out)
    # DUMMY (>= 1.9 hash is sorted by default)
  end
  def hash_ensure_sorted_each(hash_out)
...
#
# Global generator: generates/manages
# parts which are not project-local/target-related
# (i.e., manages things related to the _entire solution_ configuration)
# local generator: has a Makefile member (which contains a list of targets),
#   then generates project files
#   by iterating over the targets via a newly generated target generator each.
# target generator: generates targets.
# This is the one creating/producing the output file stream.
# Not provided by all generators (VS10 yes, VS7 no).
...
class V2C_Info_Condition
  ...
  # FIXME: Q&D interim function -
  # I don't think such raw handling should be in this data container...
  ...
  # http://en.wikipedia.org/wiki/Logical_conditional and
  ...
  def update_build_type_platform
    ...
    # For now, prefer raw build_type (VS7) only
    # in case no complex condition string is available.
...
class V2C_Tool_Base_Info
  def initialize(
    ...
    # _base_ class member to provide a mechanism
    # to intelligently translate tool (compiler, linker) configurations
    # as specified by the original build environment files
    # (e.g. compiler flags, warnings, ...)
...
# This quite likely applies to
# many other filesystem items in a project, too.
...
class V2C_ItemMetadataProviderFile < V2C_ItemMetadataProviderBase
  def do_translate(
    ...
    case metadata_key
    when 'Extension'
      metadata = File.extname(
        @item.path_relative)
    when 'Filename'
      metadata = V2C_File_Stuff::fs_location_get_filename_part_wo_ext(
        @item.path_relative)
    when 'FullPath'
...
# I believe this will be needed
# not only for file items,
# but for *many* other context-inherited VS10 build properties, too
# (e.g. %(AdditionalIncludeDirectories)).
def v2c_item_metadata_get(
  item,
  metadata_key)
  metadata = nil
  # Keep knowledge of how to assemble specifics
  # of the externally requested data
  # outside of data container classes...
  class_name = item.class.name
  provider = nil
  case class_name
  when 'V2C_Info_File'
    provider = V2C_ItemMetadataProviderFile.new(
      item)
  end
  if not provider.nil?
    metadata = provider.translate(
      metadata_key)
  else
...
class V2C_Item_List_Info < V2C_Info_Elem_Base
  ...
  def initialize(
    ...
    # VS7: the name of the filter that contains these files
    #   (FIXME: filter stuff is not really useful,
    #   should be assigning the name based on the tool type GUID!
    #   And then perhaps use the VS10 tool file type names
    #   [ClCompile, ClInclude etc.])
    @name = name
    @type = type
    @arr_items = Array.new # V2C_Info_Item elements
    @hash_items = Hash.new # V2C_Info_Item elements
  end
  attr_accessor :name
  attr_accessor :type
  ...
  def append_item(
    ...
    existing_item = @hash_items[
      path_new]
    if existing_item.nil?
      if (flags & APPEND_CASE_INSENSITIVE)
        @hash_items.each_pair { |key, value|
    ...
    if existing_item.nil?
      ...
      existing_item.extend(
        info_file)
    end
  end
  def get_item(
    item_name)
    @arr_items.each { |info_item|
      #puts "#{item_name} vs. #{info_item.path_relative}"
      if item_name.eql?(
  ...
  def get_generated_items
    array_collect_compact(@arr_items) do |info_item|
      #puts "#{info_item.path_relative} #{info_item.is_generated}"
      next if false == info_item.is_generated
      info_item.path_relative
    end
  end
  private
  def get_type(type); @type <= TYPE_NONE ? @type : TYPE_NONE end
  def get_list_type_descr()
    ITEM_LIST_DESCRIPTIONS[
      get_type(
        type)]
  end
...
class V2C_Item_Lists_Container < V2C_Info_Elem_Base
  def initialize
    ...
    @hash_item_lists = Hash.new # dito, but hashed! (serves to maintain fast lookup)
  end
  attr_reader :arr_item_lists
  def lookup_from_list_name(
    item_list_name)
    return @hash_item_lists[
      item_list_name]
  end
  def lookup_from_list_type(
    item_list_type)
    @arr_item_lists.each { |list|
      return list if item_list_type == list.type
    }
    return nil
  end
  def lookup_from_item_name(
    item_name)
    info_item = nil
    arr_item_lists.each { |item_list|
      #puts "item_list: #{item_list.name}"
      info_item = item_list.get_item(item_name)
      break if not info_item.nil?
    }
    return info_item
  end
  def append(
    item_list)
    name = item_list.name
    item_list_existing = lookup_from_list_name(
      name)
    item_list_append = item_list_existing
    if item_list_append.nil?
      register(
        item_list)
...
class V2C_Filters_Container
  def initialize
    ...
    # There's now a new ordered hash
    # which might preserve the ordering as guaranteed by an Array,
    # but we cannot use it since it's too new (Ruby 1.9!).
...
# *irrespective* of defaults defined by specific build environments
# (e.g. VS7, VS10).
...
class V2C_Project_Info < V2C_Info_Elem_Base # We need this base to always consistently get a condition element - but the VS10-side project info actually most likely does not have/use it!
  ...
  def initialize
    ...
    # Perhaps we should also be supplying a long name, too?
    ...
    # .vcproj Keyword attribute
    # ("Win32Proj", "MFCProj", "ATLProj", "MakeFileProj", "Qt4VSv1.0").
    ...
    @item_lists = V2C_Item_Lists_Container.new
    @filters = V2C_Filters_Container.new
    @arr_filtered_file_lists = nil # Array of V2C_File_Filters_Group_Info. May get post-processed from @item_lists/@filters info.
  ...
  def detect
    ...
    if true == @project_info.have_build_units
      ...
      if @arr_languages.empty?
        ...
        # We'll explicitly keep the array _empty_
        # (rather than specifying 'NONE'),
        # to give it another chance
        # via CMake's language auto-detection mechanism.
      end
    else
      # This *can* be problematic -
      # if our list fails to contain other yet-unknown language file extensions,
      # then we'd need CMake-side auto-detection,
...
# See "Format of a .vcproj File"
#   http://msdn.microsoft.com/en-us/library/2208a1f2%28v=vs.71%29.aspx
...
def vs7_create_config_variable_translation(
  ...
  str_scan_copy.scan(VS7_PROP_VAR_SCAN_REGEX_OBJ) {
    ...
    # MSVS Property / Environment variables are documented
...
EOF
    ...
    when 'SOLUTIONDIR'
      ...
      # depending on which of the possibly *multiple*
      # solution sub dir hierarchies
    ...
    when 'TARGETPATH'
      ...
      if is_known_environment_variable_convention(config_var, config_var_type_descr)
        ...
        # For now,
        # at least better directly reroute from environment variables as well:
        is_env_var = true
      end
      if true == is_env_var
...
# A block-based (yield) helper
# which given an:
# input string
#   potentially containing some variables
#   in a particular syntax
# and a
# match regex
# will then invoke a block
# for each variable name (match part) of this regex.
# Will return as a result
# the entire match-replaced string.
def transform_var_string(
  str,
  var_expr_regex)
  ...
  result = ''
  pre = post = prefix = var_name = var_value = LoopVarPreconstruct()
...
class V2C_XmlParserBase < V2C_ParserBase
  ...
  def unknown_element_attribute_value(
    ...
    value)
    unknown_something(
      "Element #{elem}\'s attribute #{attribute} value",
      value)
  end
...
# (documents many variables such as AccessedTime, Identity etc.)
def vs10_transform_item_metadata_macro_string(
  str)
  # Shortcut :)
  return str if str.nil? or not str.include?('%')
  transform_var_string(
    str,
    VS10_ITEM_METADATA_MACRO_VAR_MATCH_REGEX_OBJ) do |var|
    yield var
  end
end

class V2C_VSXmlParserBase < V2C_XmlParserBase
  # Hmm, \n at least appears in VS10 (DisableSpecificWarnings element),
  # but in VS7 as well?
  # WS_VALUE is for entries containing (and preserving!) whitespace
  # (no split on whitespace!).
  ...
  # Well, one idea would be
  # to append entries (include directories, dependencies etc.)
  # to individual list vars
  # that are being scoped within a CMake parent directory chain.
  # But these lists should be implementation details
  ...
  def parse_boolean_text(str_value)
    ...
    if not str_value.nil?
      ...
      when 'false'
        ...
        # Seems empty (whitespace-only) string
        # is VS equivalent to false, right?
...
class V2C_VSProjectSCCParser < V2C_VSXmlParserBase
  ...
  def register_scc(setting_key, setting_value)
    ...
    case setting_key
    # Hrmm, turns out having SccProjectName is no guarantee
    # that both SccLocalPath and SccProvider exist, too...
    # (one project had SccProvider missing).
    # HOWEVER,
...
class V2C_VSToolParserBase < V2C_VSXmlParserBase
  ...
  # IgnoreDefaultLibraryNames (VS7) seems to use
  # both ; and , as entry separators.
  ...
  def parse_additional_options(arr_flags, attr_options)
    ...
    # TODO: add translation table for specific compiler flag settings
    # such as MinimalRebuild:
  ...
  def bail_on_trailing_slash(path, path_descr)
    ...
    # in order to avoid
    # having that special word appear verbatim in our log output.
...
class V2C_VS7ConfigurationBaseParser < V2C_VS7ParserBase
  ...
  def parse_verify
    found = super
    # While the conditional-related information
    # is only available (parsed) once,
  ...
  def parse_file_list(vcproj_filter_xml, files_str)
    ...
    if not filter_info.nil?
      ...
      if not filter_info.name.nil?
        # Hrmm, this string match implementation
        # is very open-coded ad-hoc imprecise.
...
# Confusing!!
# For the naming and use of our data structures,
# we'll decide to adopt the VS10 structure.
...
class V2C_VS7ProjectParser < V2C_VS7ProjectParserBase
  ...
  def parse_element(subelem_xml)
    ...
    when 'Files' # "Files" simply appears to be a special "Filter" element without any filter conditions.
      # FIXME: we most likely shouldn't pass a rather global
      # "project target" object here! (pass a file info object)
...
class V2C_VS7ProjectFilesBundleParser < V2C_VSProjectFilesBundleParserBase
  ...
  def check_unhandled_file_types
    # FIXME: we don't handle now externally specified (.rules, .vsprops)
    # custom build parts yet!
...
module V2C_VS10Defines_File_List_Types
  ...
  # that these files constitute a list of items
  # *actively* excluded from
  # compile tracking (implemented by tracker.exe / tracker.dll).
...
class V2C_VS10BaseElemParser < V2C_VS10ParserBase
  ...
  def parse_attribute(setting_key, setting_value)
    ...
    when TEXT_CONDITION
      ...
      # actually had its condition parsed properly
      # (V2C_Info_Elem_Base.@condition != nil),
      # since conditions need to be parsed separately
      # by each property item class type's base class
      # (upon "Condition" attribute parsing
      # the exact property item class often is not known yet i.e. nil!!).
      # Or is there a better way to achieve common, reliable parsing
      # of that condition information?
      @have_condition = true
...
class V2C_VS10ItemGroupFilesParser < V2C_VS10BaseElemParser
  ...
  def select_list(name)
    if @cached_list_ptr.nil? || @cached_list_ptr.name != name
      ...
      @cached_list_ptr = item_lists.lookup_from_list_name(
        name)
      if @cached_list_ptr.nil?
        @cached_list_ptr = V2C_Item_List_Info.new(
          name,
          get_item_list_type(
            name))
        item_lists.append(
  ...
  def get_item_list_type(item_list_name)
    ...
    when TEXT_CLINCLUDE
      type = V2C_Item_List_Types::TYPE_CL_INCLUDES
    when TEXT_CLNODEPENDENCIES
      type = V2C_Item_List_Types::TYPE_CL_NODEPENDENCIES
    ...
    when TEXT_MIDL
      type = V2C_Item_List_Types::TYPE_MIDL
    when TEXT_EMBEDDEDRESOURCE
      type = V2C_Item_List_Types::TYPE_EMBEDDED_RESOURCES
    when TEXT_RESOURCECOMPILE
      type = V2C_Item_List_Types::TYPE_RESOURCES_RC
    when TEXT_RESOURCE
      type = V2C_Item_List_Types::TYPE_RESOURCES
    when TEXT_OBJECT
      type = V2C_Item_List_Types::TYPE_OBJECTS
    when TEXT_LIBRARY
      type = V2C_Item_List_Types::TYPE_LIBRARIES
    when TEXT_XSD
      # Removing/adding an Xsd ItemGroup element will cause
      # a MSVS10 project tab "XML Data Generator Tool" to (dis)appear.
      type = V2C_Item_List_Types::TYPE_XSD
    when TEXT__EMBEDMANAGEDRESOURCEFILE
      type = V2C_Item_List_Types::TYPE_MANAGED_RESOURCES_RC
    when TEXT_MANIFEST
    ...
    when TEXT_NONE
      type = V2C_Item_List_Types::TYPE_NONE
    when TEXT_REFERENCE
      type = V2C_Item_List_Types::TYPE_CS_REFERENCE
    when TEXT_BOOTSTRAPPERPACKAGE
      type = V2C_Item_List_Types::TYPE_CS_BOOTSTRAPPERPACKAGE
...
# and invokes them.
# V2C_VS10PropertyGroupForwarderParser / V2C_VS10ItemGroupForwarderParser
...
class V2C_VS10ToolCompilerParser < V2C_VSToolCompilerParser
  ...
  def parse_element(subelem_xml)
    ...
    when 'ObjectFileName'
       # TODO: support it - but with a CMake out-of-tree build
       # this setting is very unimportant methinks.
       skipped_element_warn(setting_key)
...
class V2C_VS10PropertyGroupForwarderParser < V2C_VS10BaseElemParser
  def parse
    ...
    if FOUND_FALSE == found
      unknown_element_attribute_value(
        @elem_xml.name,
        TEXT_LABEL,
        propgroup_label)
    end
    # Do NOT call super!
    # we're a simple forwarder class,
    # thus EVERYTHING is supposed to be "successful" for us
...
# defined by the .vcxproj file
# (i.e. the pre-existing array item will be _replaced_).
...
class V2C_VS10ProjectFilesBundleParser < V2C_VSProjectFilesBundleParserBase
  ...
  def check_unhandled_file_types
    # FIXME: we don't handle now externally specified
    # (.props, .targets, .xml files) custom build parts yet!
...
class V2C_ValidatorBase
  def validate
    validation_error(
      'Derived validate() not implemented!?')
  end
  private
  def validate_via_class_name(
...
class V2C_LinkerInfoValidator < V2C_SimpleObjectValidator
  def validate_object(
    object)
    linker_info = object
    validate_filesystem_item(
      linker_info.module_definition_file)
  end
end
...
class V2C_ConfigInfoToolsValidator < V2C_SimpleObjectValidator
  def validate_object(
    ...
    arr_linker_info.each do |linker_info_curr|
      validate_via_class_name(
        'V2C_LinkerInfoValidator',
        linker_info_curr)
    end
  end
end
...
class V2C_ProjectValidator < V2C_ValidatorBase
  ...
  def validate_project
    ...
    # FIXME: Disabled for TESTING only -
    # should re-enable a fileset check once VS10 parsing is complete.
...
class V2C_ToolProcessorBase < V2C_LoggerBase
  ...
  # given an input item (or potentially several, within a single run).
  def process(
    arr_input_items)
...
class V2C_ToolProcessorMIDL < V2C_ToolProcessorBase
  ...
  def do_transform(
    tool_info,
    arr_items)
    input_item = arr_items[0]
    tool_info.dll_data_file_name = transform_metadata(
      input_item,
      tool_info.dll_data_file_name)
    tool_info.header_file_name = transform_metadata(
      input_item,
      tool_info.header_file_name)
    tool_info.iface_id_file_name = transform_metadata(
      input_item,
      tool_info.iface_id_file_name)
    tool_info.proxy_file_name = transform_metadata(
      input_item,
      tool_info.proxy_file_name)
    tool_info.type_library_name = transform_metadata(
      input_item,
      tool_info.type_library_name)
  end
end
...
class Util_TempFilePermanentizer
  ...
  def permanentize()
    ...
    # [for chmod() comments, see our
    # $v2c_generator_file_create_permissions settings variable]
...
class V2C_GenerateIntoTempFile
  ...
  def generate
    ...
    Tempfile.open(tempfile_prefix()) { |tmpfile|
      ...
      # by writing our files (CMakeLists.txt etc.) there,
...
class V2C_BaseGlobalGenerator
  ...
  def read_mappings_includes
    # These mapping files may contain things such as
    # mapping .vcproj "Vc7/atlmfc/src/mfc"
...
def get_temporary_content_marker
  ...
  # (e.g. CMakeLists.txt)
  # from custom-written ones,
  # thus one should definitely avoid
...
def check_cmakelists_txt_type(str_cmakelists_file_fqpn)
  ...
  File.open(str_cmakelists_file_fqpn, 'r') { |f_cmakelists|
    ...
    if prior_file_was_generated_by_v2c
      ...
      # For zero-size files, the auto-generation marker check above
      # is obviously not delivering a useful result...
...
class V2C_SyntaxGeneratorBase < V2C_GeneratorBase
  ...
  # are a *potentially variable*
  # (to-be-extended - causing inconvenient parameter *shifting*)
...
class V2C_CMakeSyntaxGenerator < V2C_SyntaxGeneratorBase
  ...
  # TODO: ideally we would do single-line/multi-line splitting operation
  # _automatically_
  ...
  def put_source_group(source_group_name, arr_filters, source_files_list_var_name)
    ...
    if not arr_filters.nil?
      # WARNING: need to keep as separate array elements
      # (whitespace separator would lead to bogus quoting!)
      # And _need_ to keep manually quoted,
      # since we receive this as a ;-separated list
      # and need to pass it on unmodified.
  ...
  # CMake filesystem whitespace requirements!
  # (string _variables_ _need_ quoting)
  # However, there is a strong argument to be made for applying the quotes
  # on the _generator_ and not _parser_ side,
  # since it's a CMake syntax attribute
  ...
  def element_handle_quoting(elem)
    ...
    # For details, see REF_QUOTING:
    # "Quoting"
    #   http://www.itk.org/Wiki/CMake/Language_Syntax#Quoting
    ...
    # (this empty content might be
    # a counted parameter of a function invocation,
    # in which case unquoted syntax
    # would implicitly throw away that empty parameter!
...
# Reasoning: that class implements the border crossing
# towards specific CMake syntax,
# i.e. it is the _only one_ to know specific CMake syntax
# (well, "ideally", I have to say, currently).
# If it was the base class of the various CMake generators,
# then it would be _hard-coded_ i.e. not configurable
...
class V2C_CMakeV2CSyntaxGeneratorBase < V2C_CMakeSyntaxGenerator
  ...
  def put_converter_script_location(
    ...
    # relative argument to global V2C_MASTER_PROJECT_SOURCE_DIR
  ...
  def put_include_dir_precompiled_header(target_name, cmake_path_to_header)
    ...
    # For now sitting in LocalGenerator and not per-target handling
    # since this setting is valid for the entire directory.
    next_paragraph()
    ...
    # whenever we can help it,
    # to not introduce any troubling differences in setup.
    # It seems we need this for PCH in gcc
    # (an explicit project root include dir),
    ...
    # - reason: consistent builds
    #   (prevent build mismatch failure on *some* platforms!)
    # Note that having this include dir activated applies
    # to cases (platforms) where PCH is switched off/unavailable, too!
    # (since stdafx.h always needs to remain reachable -
    # BTW with PCH switched off on MSVC,
    # the *forced* static syntax of '#include "stdafx.h"'
    ...
    # the include path for other compilers
    # in case PCH *is* configured as active
  ...
  # BTW, we probably don't have much use
  # for the CMAKE_LINKER variable anywhere, right?
  def map_tool_name_to_cmake_platform_conditional(tool_name)
    arr_conditional_tool_platform = nil
    # For a number of platform indentifier variables, see
...
class V2C_CMakeV2CConditionGeneratorBase < V2C_CMakeV2CSyntaxGenerator
  ...
  def generate_assignments_of_build_type_variables(arr_config_info)
    ...
    # (dito with target_link_libraries() -
    # or are we supposed to create an imported target for each dependency,
    # for more precise configuration-specific library names??)
    ...
    #else... implicitly being done
    # by v2c_platform_build_setting_configure() invoked during project leadin.
    end
  end
  ...
  def get_buildcfg_var_name_of_condition(condition)
    ...
    # but I'm not convinced at all that
    # this is how things should be structured.
    build_type = condition.get_build_type()
...
# FIXME: temporarily appended a _VS7 suffix
# since we're currently changing file list generation
# during our VS10 generator work.
class V2C_CMakeFileListGenerator_VS7 < V2C_CMakeFileListGeneratorBase
  ...
  def put_file_list_recursive(files_str, parent_source_group, arr_sub_sources_for_parent)
    ...
    # TODO: CMake is said to have a weird bug
    ...
    # process sub-filters,
    # have their main source variable added to arr_my_sub_sources
    ...
    if not arr_local_sources.nil?
      ...
      if not parent_source_group.nil?
        # use list of filters if available: have it generated as
        # source_group(REGULAR_EXPRESSION "regex" ...).
...
class V2C_CMakeCompilerInfoGenerator < V2C_CMakeTargetGenerator
  ...
  def generate_compiler_info(condition, target_name, compiler_info_curr, target_config_info_curr, map_defines)
    ...
    compiler_info_curr.arr_tool_variant_specific_info.each { |compiler_specific|
      ...
      # I don't think we need this (we have per-target properties),
      # thus we'll NOT write it!
  ...
  def write_precompiled_header(condition, precompiled_header_info)
    ...
    ## FIXME: this filesystem validation should be carried out by
    ## a non-parser/non-generator validator class...
...
class V2C_CMakeProjectGenerator < V2C_CMakeTargetGenerator
  ...
  # PUBLIC_HEADER (cmake --help-property PUBLIC_HEADER),
  # PRIVATE_HEADER, HEADER_FILE_ONLY, EXTERNAL_OBJECT
  # and possibly the PUBLIC_HEADER option of the INSTALL(TARGETS) command.
  def put_file_list(
    project_info,
    arr_sub_source_list_var_names)
    # We currently have some unclean (separate) parsing
    # of file list and source group info,
    ...
    if not arr_sub_source_list_var_names.empty?
      ...
      # - and not earlier
      #   since earlier .vcproj-defined variables should be clean
      #   (not be made to contain V2C_SOURCES contents yet)
  ...
  def put_file_list_vs10(
    ...
    item_lists.arr_item_lists.each { |item_list|
      ...
      arr_sub_sources_for_parent.push(
        source_files_variable)
      arr_generated = item_list.get_generated_items
      #puts "item_list.name #{item_list.name} arr_generated #{arr_generated.inspect}"
      ...
      #puts "item_list.name #{item_list.name}, arr_generated #{arr_generated}"
      arr_file_elems = [ get_dereferenced_variable_name(source_files_variable) ]
      qualify_file_list(arr_file_elems, item_list.type)
      add_to_target = true
      file_list_generate_callback_invoker(project_name, item_list.get_list_type_user_marker(), arr_file_elems, add_to_target)
    }
  end
  ...
  def qualify_file_list(arr_file_elems, file_list_type)
    ...
    case file_list_type
    when V2C_Item_List_Types::TYPE_CL_COMPILES
    ...
    when V2C_Item_List_Types::TYPE_CL_INCLUDES
  ...
  def put_v2c_target_midl_compile(
    ...
    #
    # TODO: should use condition
    # to alternatively open-code the conditional variable
  ...
  def do_hookup_tools(
    ...
    arr_info_item_input.each do |info_item_input|
      ...
      tool_info_result = processor.transform(
        arr_processor_input)
      case tool_type
  ...
  def generate_module_definition_file_call(
    target_name,
    condition,
    ...
    write_invoke_object_conditional_v2c_function(
      '_v2c_target_linker_def_file',
      target_name,
      condition,
      arr_args_func)
  end

  # FIXME: not sure whether map_lib_dirs etc. should be passed in
  # in such a raw way -
  ...
  def put_target_and_stuff(target, arr_sub_source_list_var_names, map_lib_dirs, map_lib_dirs_dep, map_dependencies, config_info_curr, target_config_info_curr)
    ...
    arr_linker_info.each { |linker_info_curr|
      ...
      generate_module_definition_file_call(
        project_target_name,
        config_info_curr.condition,
        linker_info_curr.module_definition_file)
    }
    ...
    # which we successfully translated
    # from a bare link directory auto-link dependency
  ...
  def write_target_type(cfg_type)
    ...
    when V2C_TargetConfig_Defines::CFG_TYPE_APP
      ...
      # Well, this toggle probably is related to
      # the .vcproj Keyword attribute...
    ...
    when V2C_TargetConfig_Defines::CFG_TYPE_DLL
      ...
      # add_library() docs:
      #   "If no type is given explicitly the type is STATIC or  SHARED
      ...
      # However, since then the global BUILD_SHARED_LIBS variable
      # comes into play,
    ...
    when V2C_TargetConfig_Defines::CFG_TYPE_GENERIC
      ...
      # thus do try to establish a normal library/executable target -
      # maybe we're in luck.
      write_target_library_dynamic(target_name, string_sources_list)
    else
  ...
  def write_func_v2c_target_post_setup(
    ...
    # to a bare minimum -
    # call v2c_project_post_setup(),
    # by simply passing all parameters that are _custom_ data
    # of the current generated CMakeLists.txt file -
    # all boilerplate handling functionality
    # that's identical for each project
    # should be implemented by the v2c_project_post_setup() function
    # _internally_.
  ...
  def generate_it(generator_base, map_lib_dirs, map_lib_dirs_dep, map_dependencies, map_defines)
    ...
    if target_is_valid
      ...
      # TODO: perhaps there are useful Xcode (XCODE_ATTRIBUTE_*) properties
      # to convert?
    end # target_is_valid
...
# since e.g. compiler info is dependent on a valid project.
      ...
      # (one local directory - i.e. one single CMakeLists.txt -
    ...
    # (side note: see "ldd -u -r" on Linux
    # for superfluous link parts potentially caused by this!)
  ...
  def write_project_target_config(project_info, arr_sub_source_list_var_names, generator_base, map_lib_dirs, map_lib_dirs_dep, map_dependencies, map_defines)
    ...
    arr_config_info.each { |config_info_curr|
      ...
      gen_condition.generate(condition) do
        ...
        # FIXME: hohumm, the position of this hook include is outdated,
        # need to update it
        ...
        # Technical note: target type (library, executable, ...) in .vcproj
        # can be configured per-config
        # (or, in other words,
        # different configs are capable of generating
        # _different_ target _types_
        # for the _same_ target),
        # but in CMake this isn't possible
        # since _one_ target name maps to _one_ target type and
        # we _need_ to restrict ourselves to
        # using the project name as the exact target name
        # (we are unable to define separate PROJ_lib and PROJ_exe target names,
        # since other .vcproj file contents always link to our target
        # via the main project name only!!).
        # Thus we need to declare the target
        # _outside_ the scope of per-config handling :(
...
class V2C_CMakeGlobalBootstrapCodeGenerator < V2C_CMakeV2CSyntaxGenerator
  ...
  def put_per_scope_cmake_policies
    ...
    write_conditional_block(arr_conditional) do
      # CMP0005: manual quoting of brackets in definitions
  ...
  def put_cmake_module_path(str_conversion_root_rel)
    # try to point to cmake/Modules of the topmost directory
    # of the vcproj2cmake conversion tree.
    # This also contains vcproj2cmake helper modules
    # (these should - just like the CMakeLists.txt -
    # be within the project tree as well,
    # since someone might want to copy
    # the entire project tree including .vcproj conversions
    # to a different machine,
    # thus all v2c components should be available)
    # NOTE that V2C_MASTER_PROJECT_SOURCE_DIR is a very important variable
    # which may eventually be supported to end up _different_
    # from CMAKE_SOURCE_DIR
    # (e.g. in the case of
    # integrating _multiple_ different solution (.sln) files
    # - and their project hierarchy each! -
    # into a _higher-level_ natively CMake-based tree!!).
    # We might possibly eventually want to rename
    # V2C_MASTER_PROJECT_SOURCE_DIR into V2C_SOLUTION_ROOT_SOURCE_DIR
    # to reflect the fact that a project hierarchy has been created
    # from a solution
...
# Module for all CMake things specific to
# filesystem I/O (i.e., fs dependent generation).
# However, admittedly,
# some constants here
# may sometimes also be needed
# while generating *content*.
module CMake_Syntax_Filesystem
  CMAKELISTS_FILE_NAME = 'CMakeLists.txt'
  CMAKE_FILES_DIRECTORY_NAME = 'CMakeFiles' # The name part of the CMAKE_FILES_DIRECTORY variable
end

# Detect .vcproj files actually generated by CMake generator itself:
# This detection is a bit troublesome since
# - apart from the CMake generator
# having added its CMakeLists.txt to the source groups -
# some people may have decided to manually list
# the CMakeLists.txt in their projects -
...
#VCPROJ_IS_GENERATED_BY_CMAKE_REGEX_OBJ = %r{#{CMake_Syntax_Filesystem::CMAKELISTS_FILE_NAME}}
VCPROJ_DEF_CONTENT_CMAKE_INTDIR_REGEX_OBJ = %r{\bCMAKE_INTDIR=}
VCXPROJ_CMAKEFILES_REGEX_OBJ = %r{\b#{CMake_Syntax_Filesystem::CMAKE_FILES_DIRECTORY_NAME}\b}
VCPROJ_IS_GENERATED_BY_CMAKE_REGEX_OBJ = %r{(\bPreprocessorDefinitions\b.*\bCMAKE_INTDIR=|\b#{CMake_Syntax_Filesystem::CMAKE_FILES_DIRECTORY_NAME}\b)}
...
class V2C_CMakeFileGenerator < V2C_FileGeneratorBase
  include CMake_Syntax_Filesystem
end

class V2C_CMakeLocalFileGenerator < V2C_CMakeFileGenerator
...
def v2c_want_cmakelists_rewritten(str_cmakelists_file)
  ...
  when CMAKELISTS_FILE_TYPE_V2C_LOCAL
    ...
    error_unknown_case_value("#{CMake_Syntax_Filesystem::CMAKELISTS_FILE_NAME} type", cmakelists_type)
...
def v2c_projects_list_handle_sub_dirs(
  ...
  v2c_path_config = v2c_get_path_config(
    p_source_root.to_s)
  generated_items_dir = File.join(
    v2c_path_config.get_abs_config_dir_source_root_temp_store(),
    'generated_items')
  V2C_Util_File.mkdir_p(
    generated_items_dir)
  projects_list_file = File.join(
    generated_items_dir,
...
class V2C_ProjectPostProcess < V2C_LoggerBase
  ...
  def mark_items_as_generated(
    ...
    # One could argue that marking items as generated could be done by
    ...
    # The list of candidate items can be found
    # in the Compile and Include type at least,
    # since compiler input items may potentially be items that were
    # output items of other tools.
    arr_list_types = [
      V2C_Item_List_Info::TYPE_CL_COMPILES,
      V2C_Item_List_Info::TYPE_CL_INCLUDES,
    ]
    arr_lists_build = array_collect_compact(arr_list_types) do |list_type|
      item_lists.lookup_from_list_type(list_type)
    end
    ...
    # (implement breakout after first successful lookup?).
    info_item = LoopVarPreconstruct()
    arr_output_items.each do |candidate|
      arr_lists_build.each do |item_list_build|
        info_item = item_list_build.get_item(
          candidate)
        if info_item.nil?
          item_list_build.arr_items.each do |info_item_build|
            logger.debug "item is #{info_item_build.path_relative} candidate #{candidate}"
            if info_item_build.path_relative == candidate
              info_item = info_item_build
              break
            end
          end
        end
        if not info_item.nil?
          reason_generated = "output item of build tool #{build_tool_name}"
          mark_item_as_generated(
            info_item,
            reason_generated)
        end
        logger.debug "candidate #{candidate} info_item #{info_item.inspect}"
      end
    end
  end
  def mark_item_as_generated(
    item,
    reason)
    logger.info "Marking item #{item.path_relative} as generated (reason: #{reason})"
    item.enable_attribute(
  ...
  def process_filtered_file_lists(project_info)
    ...
    project_info.item_lists.arr_item_lists.each { |file_list|
      file_list.arr_items.each { |info_file|
  ...
  def detect_build_units(project_info)
    if not true == project_info.have_build_units
      project_info.item_lists.arr_item_lists.each { |file_list|
        arr_info_item = file_list.arr_items
        have_build_units = check_have_build_units_in_file_list(arr_info_item)
