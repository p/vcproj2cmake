  ...
  def generate
    ...
    Tempfile.open('vcproj2cmake') { |tmpfile|
      begin
        ...
        # Close file, since Fileutils.mv on an open file will barf on Windows (XP)
        tmpfile.close
      end
...
def v2c_source_root_write_projects_list_file(output_file_fqpn, output_file_permissions, arr_project_subdirs)
  ...
  begin
    ...
    File.open(tmpfile.path, 'w') { |projlistfile|
      textstream_attributes = V2C_TextStream_Attributes.new(
        $v2c_generator_indent_initial_num_spaces,
        $v2c_generator_indent_step,
        $v2c_generator_comments_level
      )
      textOut = V2C_TextStreamSyntaxGeneratorBase.new(projlistfile, textstream_attributes)
    ...
    # make sure to close the temp file as well...
    tmpfile.close
  end
...
def v2c_source_root_write_cmakelists_skeleton_file(path_cmakelists_txt, projects_list_file)
  File.open(path_cmakelists_txt, 'w') { |f_cl|
    ...
    textOut = V2C_TextStreamSyntaxGeneratorBase.new(f_cl, textstream_attributes)
